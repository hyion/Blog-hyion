    ...
    <section class="comment-section" style="transition:all .3s">
      <div class="comment-form">
        ...
        <div class="reply-name" v-if="isReply">
          ...
          <a-icon class="close-icon" title="删除" type="close" @click="cancel" />
        </div>
        <textarea class="textarea" placeholder="What do you want to say..." v-model="form.content"></textarea>

        <!-- submit loading -->
        <div class="bottom">
          <a-button @click="submitVerify">提交</a-button>
          ...
          <transition-group name="comment-item">
            ...
            >
              <div class="comment-item-box">
                <div class="head">
                  <div class="img">
                    <img :src="'/images/comment/'+item.image+'.jpg'" />
                  </div>
                  <div class="name">
                    <a>
                      {{item.name}}
                      <span v-if="item.admin">{{$store.state.data.admin_mark || '行人'}}</span>
                    </a>
                    <div class="r">
                      <div class="reply" @click="onReply(item, 1)">reply</div>
                      <span
                        class="time"
                      >{{item.time.time}} {{item.time.month.en}} {{item.time.day.on}}, {{item.time.year}}</span>
                    </div>
                  </div>
                </div>
              ...
              <div class="comments" v-if="item.child.length > 0">
                <transition-group name="comment-child">
                  <div class="item" v-for="items in item.child" :key="items.id" :data-id="items.id">
                    <div class="head">
                      <div class="img">
                        <img :src="'/images/comment/'+items.image+'.jpg'" />
                      </div>
                      <div class="name">
                        <a>
                          {{items.name}}
                          <span
                            v-if="items.admin"
                          >{{$store.state.data.admin_mark || '行人'}}</span>
                        </a>
                        <div class="r">
                          <div class="reply" @click="reply(item, 2, items)">reply</div>
                          <span
                            class="time"
                          >{{items.time.time}} {{items.time.month.en}} {{items.time.day.on}}, {{items.time.year}}</span>
                        </div>
                      </div>
                    </div>
    ...
    verifyResult(type) {
      ...
      setTimeout(() => {
        if (type) {
          ...
          this.status = 6
          setTimeout(this.onSubmit, 500) // start submit
        }
      }, 600)
    },
    // Cancel reply
      ...
      const map = {
        ...
        admin: () => {
          return {
            code: 1,
            type:
              (this.form.name == data.email_name &&
        ...
        email: () => {
          return {
            code: 2,
            type: !/^([a-zA-Z]|[0-9])(\w|\-)+@[a-zA-Z0-9]+\.([a-zA-Z]{2,4})$/.test(
              this.form.email
            ),
          }
        },
...
.comment {
  ...
  .comment-form {
    ...
    textarea {
      ...
      outline: none;
      resize: none;
      background: url('~static/images/comment/plbj.png') no-repeat bottom right;
      &:focus {
        border-color: #b9b9b9;
      }
    }
    ...
    .comment-item {
      ...
      .head {
        ...
        .img {
          ...
          margin-right: 12px;
          overflow: hidden;
          border: 1px solid rgba($color: #8590a6, $alpha: 0.5);
          img {
            width: 100%;
            height: 100%;
          }
